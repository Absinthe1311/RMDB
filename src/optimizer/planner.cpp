...
std::shared_ptr<Plan> Planner::make_one_rel(std::shared_ptr<Query> query)
{
    ...
    if(conds.size() >= 1)
    {
        ...
        while (it != conds.end()) {
            ...
            //建立join
            table_join_executors = std::make_shared<JoinPlan>(T_BlockNestLoop, std::move(left), std::move(right), join_conds);
            it = conds.erase(it);
            break;
        }
        // 根据连接条件，生成第2-n层join
        ...
        while (it != conds.end()) {
            ...
            if(left_need_to_join_executors != nullptr && right_need_to_join_executors != nullptr) {
                std::vector<Condition> join_conds{*it};
                std::shared_ptr<Plan> temp_join_executors = std::make_shared<JoinPlan>(T_BlockNestLoop, 
                                                                    std::move(left_need_to_join_executors), 
                                                                    std::move(right_need_to_join_executors), 
                                                                    join_conds);
                table_join_executors = std::make_shared<JoinPlan>(T_BlockNestLoop, std::move(temp_join_executors), 
                ...
                std::vector<Condition> join_conds{*it};
                table_join_executors = std::make_shared<JoinPlan>(T_BlockNestLoop, std::move(left_need_to_join_executors), 
    ...
    for (size_t i = 0; i < tables.size(); i++) {
        if(scantbl[i] == -1) {
            table_join_executors = std::make_shared<JoinPlan>(T_BlockNestLoop, std::move(table_scan_executors[i]), 
...
std::shared_ptr<Plan> Planner::generate_sort_plan(std::shared_ptr<Query> query, std::shared_ptr<Plan> plan)
{
    ...
    // 处理ORDER BY（支持多列）
    if (!query->orderby_cols.empty()) {
        plan = std::make_shared<SortPlan>(T_Sort, std::move(plan), 
                                         query->orderby_cols, query->orderby_desc);
    }
